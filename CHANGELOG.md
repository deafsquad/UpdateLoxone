...
### Added
- **Miniserver debug-stream capture during firmware updates** (`LoxoneUtils.MSDebugCapture`, new nested module registered in `LoxoneUtils.psd1` with 12 exported functions). A Miniserver's own debug log is the only record of what happens between the update trigger and the first HTTP answer after the restart - the `def.log` read over FTP covers the install, not the boot. `GET /dev/sps/log/<listener-ip>` makes the Miniserver send that log as UDP datagrams to `<ip>:7777` (29-byte binary header, NUL-separated ASCII records; decoded as printable runs of 8+ characters via `ConvertTo-MSDebugLines`). A parallel run now starts **one** UDP listener for all Miniservers (`Start-MSDebugCapture` in `Start-ParallelWorkflow`, before the Miniserver worker, stopped in its finally) writing `Logs/msdebug_<runstamp>.log` with every line tagged `[local-ts] [miniserver-ip]`, keeps the last 10 files, and every `Invoke-MSUpdate -CaptureDebugStream` points its Miniserver at it before the trigger (`Get-MSDebugListenerIP` picks the local address of the route toward the box), re-arms the stream once the box answers again after the restart, and disables it plus restores the extended-logging flag in its own finally. Capture is always on; it needs no flag. A listener that cannot start is a WARN and the run continues without capture. Verified on the 2026-09-22 upgrade to 17.3.9.21: k3 (10.3.98.5) delivered 101,492 datagrams / 104,892 lines including the whole reboot (`SYS Last Reboot Reason` ... `Start sudo systemctl restart pure-ftpd.service`) before the runner even saw it answer HTTP again
- **Extended logging** (`Get-/Set-MSExtendedLogging`): `/dev/cfg/loglevel` is a persisted boolean (`5` is clamped to `0` by the firmware, verified live 2026-09-02 on e1 17.2.8.28; it self-expires after 14 days). Flag 1 roughly doubles the rate and adds subsystems absent at 0 (`OTAU check`, `mDNS request`, `CUDPsocket::ReceiveFrom`, `MS #Send`). The runner sets it to 1 for the update, verifies by read-back, and restores the previous value afterwards; because the value PERSISTS on the Miniserver, a failed restore is a WARN naming the manual fix (`GET /dev/cfg/loglevel/<old>`). A flag that cannot be read (no right or no endpoint) logs INFO and capture proceeds at whatever level the operator set in Config
- **Capture coverage is stated in the final summary, the toast and the log**: `MS debug capture covered N of M Miniservers`, followed by one line per Miniserver that was NOT captured with its reason (`Get-MSDebugCaptureSummary`), plus the list of boxes whose stream was re-armed after the restart. Prompted by the 2026-09-22 run, which covered ONE of three: home (192.168.178.2) and e1 (192.168.2.210) answered `Code 403` on both the stream enable and the extended-logging flag because the plain `update` user lacks the debug-log right, and the only trace was an INFO line at 02:09 - found the next morning, after the post-reboot window the operator wanted to read on home had passed. The file held 104,892 k3 lines, 14 e1 lines (its boot banner) and 0 home lines. A refusal now logs at WARN when it happens AND once more in the coverage line after the workflow, and the reason carries the fix (`Get-MSDebugRightHint`): the right comes with the **'LOXONE Config' permission** of the user in Loxone Config (`loxq grant update "LOXONE Config"`). k3's `update` user already had it; home and e1 were granted it from the loxq side on 2026-09-22 (home 40cff0f, e1 785c723), so the next run should read `debug stream enabled -> <ip>:7777` for all three. `Request-MSDebugStream` returns the outcome with `Code` and `Reason`; `Enable-MSDebugStream` stays as its boolean wrapper. `Invoke-MSUpdate`'s result gained `DebugCapture` (`IP`, `Enabled`, `Rearmed`, `Reason`), `Start-MiniserverWorker` carries it into the pipeline results on both the success and the failure path, and the main script collects it per Miniserver that actually went through an update attempt (already-current boxes are not counted). All-covered logs at INFO with a 📡 line in the summary; anything less is WARN and ⚠️
- `-RunPostUpdateHook` on `UpdateLoxone.ps1`: run the post-update hook although no Miniserver changed in this run (see the fence under Changed)
- `-Force` on `UpdateLoxone.ps1`: override a FAILED precheck and run the pipeline anyway. The prechecks still run and still log their FAIL with expected/actual, each overridden check gets its own `OVERRIDDEN:` WARN line, the summary WARN says `PROCEEDING ANYWAY`, and the final summary carries `⚠️ FORCED past failed precheck: <reason>` so a forced run can never read like a clean one. The blocked-path WARN now ends with `Re-run with -Force to override.`
- `Test-LoxoneHostIsPackaged` in `LoxoneUtils.Toast`: P/Invoke `GetCurrentPackageFullName` (rc 15700 = no package identity). The BurntToast init line now logs `version=... | host=<exe> | packaged=<bool>`, so a toast that shows the PowerShell symbol instead of Loxone's can be attributed from the log
- Tests: `LoxoneUtils.MSDebugCapture.Tests.ps1` (datagram decoding, LL-XML parsing, Basic auth incl. SecureString, flag read-back, 403 handling with the 'LOXONE Config' hint, the coverage summary line, a live loopback listener on port 27777); `LoxoneUtils.SyntaxInvariants.Tests.ps1` (every module and the main script parse, and no `try`/`catch`/`finally` keyword sits detached from a block - the shape that runs as a *command* and explodes only on the path that reaches it; also asserts the capture teardown lives in `Invoke-MSUpdate`); `LoxoneUtils.ParallelWorkflow.StatusMonotonicity.Tests.ps1` and `LoxoneUtils.Network.CRC32.Tests.ps1`

### Changed
- **Post-update hook is fenced**: it runs only when this run actually changed a Miniserver's firmware (a Miniserver target with `UpdatePerformed` and a status starting `UpdateSuccessful`). A run that finds every Miniserver already on the target build, or fails to update any, logs `post-update hook skipped: no Miniserver firmware changed in this run (N already current / not updated). Use -RunPostUpdateHook to run it anyway.` and finishes without it; previously the hook (a 17-minute loxq reprocess on this fleet) ran on every invocation. `-Force` and `-RunPostUpdateHook` bypass the fence; `-SkipPostUpdateHook` still wins over both. The log says which of the three decided, and a wanted hook lists the updated Miniservers as `<name> -> <version>`
- **BurntToast: one code path.** The 0.x branch (`-AppId` on `Submit-/Update-BTNotification`, preferred on PowerShell 5.1) is gone from `Initialize-Toast`, `Update-Toast` and `Show-FinalStatusToast`; the loader in `UpdateLoxone.ps1` installs (`-MinimumVersion 1.0.0`) or imports the highest BurntToast **1.0+** on every edition, an already-loaded or too-old 0.x module is a WARN, and branding always comes from the AppUserModelId registration (touch-then-repair). Verified 2026-09-02 with BurntToast 1.1.0: the Loxone symbol shows from powershell.exe and from MSI/ZIP pwsh; an MSIX/Store-packaged pwsh (winget's PowerShell Preview) cannot be branded at all because Windows attributes its toasts to the package - the init now WARNs `Toast host is MSIX/Store-packaged ... Run from an unpackaged PowerShell` instead of silently showing the PowerShell symbol, while still refreshing the registration for later unpackaged runs
- Loxone App shortcuts (Start Menu and Desktop, in `Start-ComponentWorker`, `Start-InstallWorker` and `Invoke-InstallLoxoneApp`) no longer carry `--disable-gpu --disable-software-rasterizer`; the flag forced the app to software-render
- Bumped winget package manifests (`deafsquad.UpdateLoxone`) to version 0.9.5 with the new installer URL, SHA256 checksum and release date 2026-09-22. The three manifests are now written without the UTF-8 BOM that 0.9.4 introduced

### Fixed
- **Debug stream and extended-logging flag leaked past the update.** `Disable-MSDebugStream` and the flag restore sat in `Test-LoxoneMiniserverUpdateLevel`'s finally, where the flags are the module-scope `$false` and the `*InInvoke` variables do not exist, so the teardown never ran. Measured 2026-09-04: K3 (production) had streamed ~77 datagrams/s to this workstation for 58 hours after its 2026-09-02 update and its loglevel was still 1 two days later - the copy the polling loop re-arms after the restart is exactly the one that leaked. The teardown now lives in `Invoke-MSUpdate`'s finally - the function that arms the stream - runs on every exit path including a throw, and a stream or flag it cannot confirm off is a WARN naming the manual fix (`GET /dev/sps/log`, `GET /dev/cfg/loglevel/<old>`)
- Hook output filter truncated multi-line sentences: `$notable` matched per line, and the hook's tools write multi-line prose, so a match on the first line logged a clause with no predicate and demoted the rest to DEBUG (seen live: `the 81 TEXT comparisons are all against the EMPTY STRING - the only failure they can` followed by an unrelated INFO line). After a notable or failure line, up to 3 following lines are logged at INFO while the text is unfinished (no terminal `.`/`!`/`?`; a trailing `:` counts as unfinished because it promises a list)

## [0.9.5] - 2026-09-22 12:29:12

### Added
- **Miniserver debug-stream capture during firmware updates** (`LoxoneUtils.MSDebugCapture`, new nested module). A Miniserver's own debug log is the only record of what happens between the update trigger and the first HTTP answer after the restart - the `def.log` read over FTP covers the install, not the boot. `GET /dev/sps/log/<listener-ip>` makes the Miniserver send that log as UDP datagrams to `<ip>:7777` (29-byte binary header, NUL-separated ASCII records; decoded as printable runs of 8+ characters). A parallel run now starts **one** UDP listener for all Miniservers (`Start-MSDebugCapture`, before the Miniserver worker, stopped in `Start-ParallelWorkflow`'s finally) writing `Logs/msdebug_<runstamp>.log` with every line tagged `[local-ts] [miniserver-ip]`, keeps the last 10 files, and every `Invoke-MSUpdate -CaptureDebugStream` points its Miniserver at it before the trigger, re-arms the stream once the box answers again after the restart, and disables it plus restores the extended-logging flag in its own finally. Capture is always on; it needs no flag. Verified on the 2026-09-22 upgrade to 17.3.9.21: k3 (10.3.98.5) delivered 101,492 datagrams / 104,892 lines including the whole reboot (`SYS Last Reboot Reason` ... `Start sudo systemctl restart pure-ftpd.service`) before the runner even saw it answer HTTP again
- **Extended logging** (`Get-/Set-MSExtendedLogging`): `/dev/cfg/loglevel` is a persisted boolean (`5` is clamped to `0` by the firmware, verified live 2026-09-02 on e1 17.2.8.28; it self-expires after 14 days). Flag 1 roughly doubles the rate and adds subsystems absent at 0 (`OTAU check`, `mDNS request`, `CUDPsocket::ReceiveFrom`, `MS #Send`). The runner sets it to 1 for the update, verifies by read-back, and restores the previous value afterwards; because the value PERSISTS on the Miniserver, a failed restore is a WARN naming the manual fix (`GET /dev/cfg/loglevel/<old>`)
- **Capture coverage is stated in the final summary, the toast and the log**: `MS debug capture covered N of M Miniservers`, followed by one line per Miniserver that was NOT captured with its reason (`Get-MSDebugCaptureSummary`). Prompted by the 2026-09-22 run, which covered ONE of three: home (192.168.178.2) and e1 (192.168.2.210) answered `Code 403` on both the stream enable and the extended-logging flag because the plain `update` user lacks the debug-log right, and the only trace was an INFO line at 02:09 - found the next morning, after the post-reboot window the operator wanted to read on home had passed. The file held 104,892 k3 lines, 14 e1 lines (its boot banner) and 0 home lines. A refusal now logs at WARN when it happens AND once more in the coverage line after the workflow, and the reason carries the fix: the right comes with the **'LOXONE Config' permission** of the user in Loxone Config (`loxq grant update "LOXONE Config"`). k3's `update` user already had it; home and e1 were granted it from the loxq side on 2026-09-22 (home 40cff0f, e1 785c723), so the next run should read `debug stream enabled -> <ip>:7777` for all three. `Request-MSDebugStream` returns the outcome with `Code` and `Reason`; `Enable-MSDebugStream` stays as its boolean wrapper. `Invoke-MSUpdate`'s result gained `DebugCapture` (`IP`, `Enabled`, `Rearmed`, `Reason`), the Miniserver worker carries it into the pipeline results, and the main script collects it per Miniserver that actually went through an update attempt (already-current boxes are not counted). All-covered logs at INFO with a 📡 line in the summary; anything less is WARN and ⚠️
- `-RunPostUpdateHook` on `UpdateLoxone.ps1`: run the post-update hook although no Miniserver changed in this run (see the fence under Changed)
- `-Force` on `UpdateLoxone.ps1`: override a FAILED precheck and run the pipeline anyway. The prechecks still run and still log their FAIL with expected/actual; the WARN says `PROCEEDING ANYWAY`, and the final summary carries `⚠️ FORCED past failed precheck: <reason>` so a forced run can never read like a clean one
- `Test-LoxoneHostIsPackaged` in `LoxoneUtils.Toast`: P/Invoke `GetCurrentPackageFullName` (rc 15700 = no package identity). The BurntToast init line now logs `version=... | host=<exe> | packaged=<bool>`, so a toast that shows the PowerShell symbol instead of Loxone's can be attributed from the log
- Tests: `LoxoneUtils.MSDebugCapture.Tests.ps1` (datagram decoding, LL-XML parsing, Basic auth incl. SecureString, flag read-back, 403 handling with the 'LOXONE Config' hint, the coverage summary line, a live loopback listener on port 27777); `LoxoneUtils.SyntaxInvariants.Tests.ps1` (every module and the main script parse, and no `try`/`catch`/`finally` keyword sits detached from a block - the shape that runs as a *command* and explodes only on the path that reaches it; also asserts the capture teardown lives in `Invoke-MSUpdate`); `LoxoneUtils.ParallelWorkflow.StatusMonotonicity.Tests.ps1` and `LoxoneUtils.Network.CRC32.Tests.ps1`

### Changed
- **Post-update hook is fenced**: it runs only when this run actually changed a Miniserver's firmware (a Miniserver target with `UpdatePerformed` and a status starting `UpdateSuccessful`). A run that finds every Miniserver already on the target build, or fails to update any, logs `post-update hook skipped: no Miniserver firmware changed in this run (N already current / not updated). Use -RunPostUpdateHook to run it anyway.` and finishes without it; previously the hook (a 17-minute loxq reprocess on this fleet) ran on every invocation. `-Force` and `-RunPostUpdateHook` bypass the fence; `-SkipPostUpdateHook` still wins over both. The log says which of the three decided
- Hook output filter carries the rest of a sentence: `$notable` matched per line, and the hook's tools write multi-line prose, so a match on the first line logged a clause with no predicate and demoted the rest to DEBUG. After a notable line, up to 3 following lines are logged at INFO while the text is unfinished (no terminal `.`/`!`/`?`; a trailing `:` counts as unfinished because it promises a list)
- **BurntToast: one code path.** The 0.x branch (`-AppId` on `Submit-/Update-BTNotification`, preferred on PowerShell 5.1) is gone; the loader installs/imports the highest BurntToast **1.0+** on every edition and branding always comes from the AppUserModelId registration (touch-then-repair). Verified 2026-09-02 with BurntToast 1.1.0: the Loxone symbol shows from powershell.exe and from MSI/ZIP pwsh; an MSIX/Store-packaged pwsh (winget's PowerShell Preview) cannot be branded at all because Windows attributes its toasts to the package - the init now WARNs `Toast host is MSIX/Store-packaged ... Run from an unpackaged PowerShell` instead of silently showing the PowerShell symbol
- `Disable-MSDebugStream` / flag restore moved into `Invoke-MSUpdate`'s finally - the function that arms the stream. It used to sit in `Test-LoxoneMiniserverUpdateLevel`'s finally, where the flags are the module-scope `$false` and the `*InInvoke` variables do not exist, so it never ran. Measured 2026-09-04: K3 (production) had streamed ~77 datagrams/s to this workstation for 58 hours after its 2026-09-02 update and its loglevel was still 1 two days later - the copy the polling loop re-arms after the restart is exactly the one that leaked
- Loxone App shortcuts no longer carry `--disable-gpu --disable-software-rasterizer`; the flag forced the app to software-render
